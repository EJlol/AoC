		...
		return input.stream()
				.mapToInt(line -> {
					SimplePair<Range<Integer>> pair = StringHelper.splitAtCharacterInTwo(line, ',').map(value -> {
						SimplePair<Integer> rangePair = StringHelper.splitAtCharacterInTwo(value,'-').map(Integer::parseInt);
						return Range.openClosed(rangePair.getLeft(), rangePair.getRight());
					});
					return pair.getLeft().isConnected(pair.getRight()) || pair.getRight().isConnected(pair.getLeft()) ? 1 : 0;
				})
				.sum();
	}
