...
public class Task implements TaskInterface<Integer> {
	...
	public Integer executeTask1(List<String> input) {
		tunnels = input.stream()
				...
				.map(simplifiedInput -> ParameterStringHelper.getParameters(simplifiedInput, "Valve {name} has flow rate={flowRate}; tunnel lead to valve {tunnels}"))
	...
	private record Tunnel(String name, int flowRate, List<String> connections) {
